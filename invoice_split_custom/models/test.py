...
class invoiceInheritance(models.Model):
    ...
    def onchange_current_currency_rate(self):
        ...
        for lines in self.invoice_line_ids:
            ...
            if lines.itemprice:
                lines.itemprice = float(self.current_currency_rate) * float(lines.amtinusd)
...
class SplitInitialInvoice(models.Model):
    ...
    invoice_summary_line = fields.One2many('invoice.summary.line', 'splitted_invoice',
                                           compute='_compute_invoice_summary_line', store=True,
                                           string='Invoice Summary Line')
    ...
    total_discount_val = fields.Float(string="Total Discount Value", compute='_compute_total_discount_val', store=True)
    total_invoice_val = fields.Float(string="Total Invoice Value", compute='_compute_total_invoice_val', store=True)
    total_value_netamtinusdfob = fields.Float(string="Total Net Amount in USD",
                                              compute='_compute_total_value_netamtinusdfob', store=True)

    total_usd_words = fields.Char(string="Total Net Amount in USD", compute='_compute_amount_totalusd_words',
                                  store=True)
    total_inr_words = fields.Char(string="Total Net Amount in INR", compute='_compute_amount_totalinr_words',
                                  store=True)
    ...
    def _compute_invoice_summary_line(self):
        for record in self:
            for order_line in record.order_line:
                ...
                check_inv_sum_line = self.env['invoice.summary.line'].search(
                    [('gst_rate', '=', order_line.gstrate), ('splitted_invoice', '=', order_line.split_invoice.id)],
                    limit=1)
    ...
    def retrieve_data_from_sftp(self):
        ...
        remote_path = "/opt/EasternIRNTest/ODOO-TEST-116.xlsx"
    ...
    def generate_irn(self):
        ...
        if self.shippingbillnumber:
            ...
            headers = {"Content-type": "application/json",
                       "x-cleartax-auth-token": "1.8deef871-4db0-425f-80ae-7ef08034ba05_6f5ad64acceb37bc8a75be8801ad788342b53f158809db451a0acc3a7daacb3a",
                       "x-cleartax-product": "EInvoice", "owner_id": "a41966ae-8da5-4240-8721-96d3f8c89f24",
            ...
            try:
                ...
                if content[0]['govt_response']['Success'] == 'Y':
                    ...
                    self.govt_log = 'Status: ' + content[0]['govt_response']['Success'] + ', AckNo: ' + str(
                        content[0]['govt_response']['AckNo']) + ', AckDate: ' + str(
                        content[0]['govt_response']['AckDt'])
                    qr = qrcode.QRCode(
                        version=1,
                        error_correction=qrcode.constants.ERROR_CORRECT_L,
                        box_size=10,
...
class AccountMove(models.Model):
    ...
    splitted_invoice_count = fields.Integer(string='Invoice Count', compute='_compute_splitted_invoices_count',
                                            readonly=True)
